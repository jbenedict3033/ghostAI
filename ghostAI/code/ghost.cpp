...
Ghost::Ghost(int X, int Y, std::string path,
    ...
    releaseTime = SDL_GetTicks() + penTime * order; //define pen
                                                    //release time
    ...
    for (int MOVEMENT = RIGHT; MOVEMENT < MOVE_NUM; //set utilities
        MOVEMENT++) movements[MOVEMENT] = .5;       //to starting
    for (int ACTION = GET_TO_DOOR; ACTION <         //values
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
        ...
        case GET_TO_DOOR:                           //action:
            ...
            if (SDL_GetTicks() >= releaseTime &&    //check if ghost
                distance(pos.x, pos.y, targetPos.x, //can leave pen
                targetPos.y) == TILE_SIZE) {        //yet
                pos.x = targetPos.x;
                pos.y = targetPos.y;
...
bool Ghost::pacCollision() {
    if ((pos.x == pacPos.x) && (pos.y == pacPos.y)) {
        ...
        if (actionStatus == AVOID_PACMAN) {         //checks if
...
void Ghost::respawn() {
    ...
    actionStatus = GET_TO_DOOR;                     //reset chosen
    releaseTime = SDL_GetTicks() + penTime;         //action and
                                                    //how long the
                                                    //ghost is stuck
                                                    //in the pen
}
